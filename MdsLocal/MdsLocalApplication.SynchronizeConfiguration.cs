...
namespace MdsLocal
{
    ...
    {
    //    // The running service should match the current configuration
    //    public static async Task SynchronizeService(CommandContext commandContext, State state, string serviceName)
    //    {
    //        var nodeConfiguration = await commandContext.Do(GetLocalKnownConfiguration);

    //        var serviceConfiguration = nodeConfiguration.SingleOrDefault(x => x.ServiceName == serviceName);
