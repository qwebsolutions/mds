...
namespace MdsInfrastructure
{
    ...
    {
        public static Var<HyperNode> OkButton<TState>(this BlockBuilder b)
            where TState: IEditPage<TState>
        {
            var popPage = b.MakeAction<TState>((BlockBuilder b, Var<TState> state) =>
            {
                b.PopView(state, x => x.EditStack);
                return b.Clone(state);
            });

            var okButton = b.CommandButton<TState>(b =>
            {
                b.Set(x => x.Label, "OK");
